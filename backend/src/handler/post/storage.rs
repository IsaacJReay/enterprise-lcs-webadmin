use crate::{
    db, handler, linux,
    structs::{MakeDirectoryArgs, MoveOrCopyArgs, PartUUID},
};
...
pub async fn post_storage_device_copy_or_move(
    req: HttpRequest,
    args_vec: web::Json<MoveOrCopyArgs>,
) -> Result<HttpResponse> {
...
pub async fn post_storage_device_directory_creation(
    req: HttpRequest,
    directory_info: web::Json<MakeDirectoryArgs>,
) -> Result<HttpResponse> {
    let (_username, _password) = handler::handle_validate_token_response(&req)?;
...
pub async fn post_storage_device_unmount(
    req: HttpRequest,
    uuid_struct: web::Json<PartUUID>,
) -> Result<HttpResponse> {
    let (_username, password) = handler::handle_validate_token_response(&req)?;

    let full_dev_path = format!(
        "/dev/{}",
        db::storage::query_from_storage_table(None, Some(&uuid_struct.drive_partuuid)).0
    );
