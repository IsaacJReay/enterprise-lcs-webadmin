use std::{convert::TryInto, isize, usize};
...
pub fn to_binary(ip: String) -> usize {
    ...
    let concated_binary_ip = padding_convert(splited_ip[0].to_string())                  //First Octet of IP
    +
    ...
    let binary: usize = isize::from_str_radix(&concated_binary_ip, 2)
        .unwrap()
        .try_into()
        .unwrap();
    binary
