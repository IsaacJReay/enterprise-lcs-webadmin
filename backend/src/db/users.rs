...
pub fn insert_into_auth_table(username: &str, encrypted_password: &str, session_id: &str, iat: u64){
    let connection = Connection::open("/tmp/lcs.db").unwrap();

    let mut check_overflowed_statement = connection.prepare(
        "SELECT COUNT(UserName) FROM tblAuth WHERE UserName=:name",
    ).unwrap();
    let mut rows = check_overflowed_statement.query(&[(":name", username)]).unwrap();
    if rows.next().unwrap().unwrap().get::<usize, u64>(0).unwrap() > SESSION_LIMIT-1 { // Minus 1 because System Count from Zero and Human Count from One
        connection.execute(
...
pub fn get_pass_from_tbl(username: &str, session_id: &str, iat: u64) -> Result<String, (u32, String)> {
    let connection = Connection::open("/tmp/lcs.db").unwrap();

    let mut check_overflowed_statement = connection.prepare(
        "SELECT CryptedPass FROM tblAuth WHERE UserName=? AND SessionID=? AND IAT=?",
    ).unwrap();
    let mut rows = check_overflowed_statement.query(params![username, session_id, iat]).unwrap();
...
pub fn login(username: &str, password: &str) -> Result<String, String> {
    ...
    let encrypted_password = security::encrypt(password.to_string(), security::padding_convert(&random_key));

    insert_into_auth_table(username, &encrypted_password, &current_session_id, timestamp);
...
pub fn extract_claims_from_token(token: &str) -> Result<Claims, (u32, String)>{ 
    ...
    let token_message = match decode::<Claims>(&token, &DecodingKey::from_secret(DECRYPT_KEY.as_ref()), &validation) {
        Ok(token) => Ok(token) ,
...
pub fn extract_auth_from_claims(claims: &Claims) -> Result<(String, String), (u32, String)> {
    ...
    let decrypted_password = match security::decrypt(encrypted_password, security::padding_convert(&claims.auth)) {
