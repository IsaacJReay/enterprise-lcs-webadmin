...
pub fn read_wlan_networkd() -> (
    ...
    let struct_ipv4network = Ipv4Network::new(router_ip.parse().unwrap(), prefix_length).unwrap();
...
pub fn read_eth0() -> (String, String, String, String) {
    ...
    let eth0_ipaddress =
        r#"ip address | grep eth0 |grep inet | awk -F' ' '{printf $2}' |awk -F '/' '{printf $1}'"#;
    ...
    let (_code, macaddr_output, _error) =
        run_script::run_script!(&format!("{}", eth0_macaddress), &vec![], &options).unwrap();

    let (_code, ipaddr_output, _error) =
        run_script::run_script!(&format!("{}", eth0_ipaddress), &vec![], &options).unwrap();

    let (_code, prefix_output, _error) =
        run_script::run_script!(&format!("{}", eth0_prefixaddr), &vec![], &options).unwrap();
...
pub fn read_wlan0() -> (String, String, String) {
    ...
    let wlan0_ipaddress =
        r#"ip address | grep wlan0 |grep inet | awk -F' ' '{printf $2}' |awk -F '/' '{printf $1}'"#;
    let wlan0_prefixaddr = r#"ip address | grep wlan0 |grep inet | awk -F' ' '{printf $2}'"#;

    let (_code, macaddr_output, _error) =
        run_script::run_script!(&format!("{}", wlan0_macaddress), &vec![], &options).unwrap();

    let (_code, ipaddr_output, _error) =
        run_script::run_script!(&format!("{}", wlan0_ipaddress), &vec![], &options).unwrap();

    let (_code, prefix_output, _error) =
        run_script::run_script!(&format!("{}", wlan0_prefixaddr), &vec![], &options).unwrap();
