...
use crate::{
    linux,
    structs::{
        ContentServerUpdate, DirectoryInfo, HostapdParam, ItemMetaData, PartialRangeIter,
        PathPartition, StaticWiredNetworkParam, WirelessNetworkParam,
    },
    CHUNK_SIZE,
};
...
pub fn insert_update_information_to_toml(
    status: (bool, bool),
    new_update_id: &str,
    new_update: &Value,
    is_sys_update: bool,
...
pub fn remove_update_information_from_toml(
    status: (bool, bool),
    update_id: &str,
    is_sys_update: bool,
) {
...
pub fn generate_file_system_struct(linux_path: &str, drive_label: &str) -> DirectoryInfo {
    ...
    let mut main_directory_info =
        DirectoryInfo::new(linux_path, Some(ItemMetaData::new(root_path_metadata)));

    for path in root_path {
        let entry_path = path.as_ref().unwrap().path();

        let current_metadata = ItemMetaData::new(match entry_path.metadata() {
            Ok(result) => result,
...
pub fn build_tree(
    current_node: &mut DirectoryInfo,
    current_parts_list: &Vec<String>,
    current_metadata: Option<ItemMetaData>,
    current_parts_depth: usize,
) {
    if current_parts_depth < current_parts_list.len() {
        ...
        let mut new_node = match current_item_name.starts_with(".") {
            ...
            false => match current_node.find_child(&current_item_name) {
                ...
                None => current_node.add_child(DirectoryInfo::new(
                    &current_item_name,
...
pub fn config_hostapd(password: &str, hostapdparam: HostapdParam) -> Result<(), String> {
    let conf: String = templates::gen_hostapd_conf(
        &hostapdparam.ssid,
        hostapdparam.hide_ssid,
        &hostapdparam.hw_mode,
        &hostapdparam.channel,
        hostapdparam.wpa,
        &hostapdparam.passphrase,
        hostapdparam.hw_n_mode,
        hostapdparam.qos,
    );
...
pub fn config_systemd_networkd_wireless(
    password: &str,
    wlanparam: WirelessNetworkParam,
) -> Result<(), String> {
    ...
    let named_conf_acl: String =
        templates::gen_named_conf_acl(&wlanparam.router_ip, &wlanparam.netmask);
...
pub fn config_systemd_networkd_wired_static(
    password: &str,
    staticwirednetworkparam: StaticWiredNetworkParam,
) -> Result<(), String> {
