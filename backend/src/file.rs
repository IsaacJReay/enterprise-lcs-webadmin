...
pub fn config_hostapd(hostapdparam: HostapdParam) -> (bool, bool, bool, bool){
    ...
    let (code,_output,_error) = linux::mvfile(&password, "hostapd.conf", "/etc/hostapd");
    match &code {
...
pub fn config_systemd_networkd_wireless(wirelessnetworkparam: WirelessNetworkParam) -> (bool, bool, bool, bool, bool, bool, bool){
    ...
    // Match Create File statuses
    {
        ...
        let (code,_output,_error) = linux::mvfile(&password, "20-wireless.network", "/etc/systemd/network/");
        match &code {
            0 => move_networkd_status = true,
            _ => move_networkd_status = false,
        }

        let (code,_output,_error) = linux::mvfile(&password, "named.conf.acl", "/etc/");
        match &code {
            0 => move_acl_status = true,
            _ => move_acl_status = false,
        }

        let (code,_output,_error) = linux::mvfile(&password, "named.conf.options", "/etc/");
        match &code {
...
pub fn config_named() -> (bool, bool) {
    ...
    let (code,_output,_error) = linux::mvfile(&password, "named.conf", "/etc/");
    match &code {
        0 => move_conf_status = true,
        _ => move_conf_status = false,
    }

    let (code,_output,_error) = linux::mvfile(&password, "named.conf.logging", "/etc/");
    match &code {
        0 => move_logging_status = true,
        _ => move_logging_status = false,
    }
    
    let (code,_output,_error) = linux::mvfile(&password, "named.conf.zones", "/etc/");
    match &code {
...
pub fn config_systemd_networkd_wired_static(staticwirednetworkparam: StaticWiredNetworkParam) -> (bool, bool, bool, bool) {
    ...
    let (code, _output, _error) = linux::mvfile(&password, "20-wired.network", "/etc/systemd/network/");
...
pub fn config_name_conf_external_zones() -> (bool, bool, bool, bool, bool) {
    ...
    let (code, _output, _error) = linux::rmfile(&password, "/etc/named.conf.external.zones");
    match code {
        0 => cleanup_exzone_status = true,
        _ => cleanup_exzone_status = false,
    }

    let (code, _output, _error) = linux::rmfile(&password, "/var/named/*.external.zone");
    match code {
    ...
    let (code, _ouput, _error) = linux::mvfile(&password, "named.conf.external.zones", "/etc/");
    match code {
    ...
    for increments in 0..zone_vec.len(){
        let filename: String = zone_vec[increments].domain_name.to_owned() + ".external.zone";
        let (code, _ouput, _error) = linux::mvfile(&password, &filename, "/var/named/");
        match code {
            0 => (),
...
pub fn config_var_named_external_zones(zone_vec: Vec<PartialZoneRecords>) -> (bool, bool, bool, bool){
    ...
    let (code, _output, _error) = linux::mvfile(&password, &&filename, "/var/named");
    match code {
